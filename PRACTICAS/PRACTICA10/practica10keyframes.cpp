...
//Librerias para trabajar con el .txt de keyframes
...
float reproduciranimacion, habilitaranimacion, guardoFrame, reinicioFrame, ciclo, ciclo2, contador, lecturaTxt = 0;
...
int FrameIndex = 12;			//El número de cuadros guardados actualmente desde 0 para no sobreescribir

//const char* KF_PATH = "keyframes.txt";  // archivo de keyframes en texto
const char* KF_PATH = R"(C:\Users\stril\OneDrive\Escritorio\LabGrafica\practica10\practica10\keyframes.txt)";
...
bool loadKeyframesFromFile(const char* path) {
	...
	if (!in) {
		printf("Aviso: no se pudo abrir '%s'. Verifica la carpeta de trabajo.\n", path);
		return false;
	}
	FrameIndex = 0; // reiniciamos

	std::string line;
	int count = 0;

	while (std::getline(in, line)) {
		if (line.empty() || line[0] == '#') continue; // comentarios/vacías

		std::istringstream ss(line);
		float x, y, g;
		if (ss >> x >> y >> g) {
			if (FrameIndex >= MAX_FRAMES) {
				printf("Límite MAX_FRAMES alcanzado. Se ignorarán frames extra.\n");
				break;
			}
	...
	if (count < 2) {
		printf("No se encontraron frames válidos en '%s'.\n", path);
		return false;
	}

	// Evidencia en consola
	printf("Leídos %d keyframes desde '%s':\n", count, path);
	for (int i = 0; i < FrameIndex; ++i) {
		printf("[%02d] x=%.2f  y=%.2f  giro=%.2f\n",
			i, KeyFrame[i].movAvion_x, KeyFrame[i].movAvion_y, KeyFrame[i].giroAvion);
	}
	return true;
}

void saveKeyframesToFile(const char* path) {
	...
	out << "# Keyframes generados desde OpenGL\n";
	out << "# Formato: movX movY giro\n";

	for (int i = 0; i < FrameIndex; ++i) {
		out << KeyFrame[i].movAvion_x << " "
...
int main()
{
	...
	//////Regreso al punto inicial pero girado 180 grados////
	...
	loadKeyframesFromFile(KF_PATH);  // <- prueba de lectura al arrancar
		...
		printf("7.-Presiona O para guardar todos los keyframes actuales en el archivo TXT\n");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::rotate(model, giroAvion * toRadians, glm::vec3(0.0f, 1.0f, 0.0f)); //Giro del helicoptero
...
void inputKeyframes(bool* keys)
{
	...
	// ---- NUEVO: cargar keyframes desde TXT (tecla I)
	if (keys[GLFW_KEY_I])
	{
		if (lecturaTxt < 1)
		{
			if (loadKeyframesFromFile(KF_PATH)) {
				resetElements();       // como cuando presionas espacio: deja listo para reproducir
				printf("Listo: keyframes cargados desde TXT. Reproduce con la barra espaciadora.\n");
			}
			lecturaTxt++;
